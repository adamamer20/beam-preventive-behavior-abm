...
from beam_abm.decision_function.derivations import (  # noqa: E402  -- env set above
    drop_redundant_predictors,
    load_derivations,
)
from beam_abm.decision_function.io import load_column_types_with_reference  # noqa: E402  -- env set above
from beam_abm.decision_function.missingness import (  # noqa: E402  -- env set above
...
from beam_abm.decision_function.modeling import (  # noqa: E402  -- env set above
    ModelBlock,
    ModelResult,
    ModelSpec,
    _compute_pseudo_r2,
...
from beam_abm.decision_function.taxonomy import (  # noqa: E402  -- env set above
...
def _write_ref_choice_model_plan(plan_records: list[dict[str, object]], output_path: Path) -> None:
    ...
    repo_root = next(
        (parent for parent in this_file.parents if (parent / "pyproject.toml").exists()),
        None,
    )
    if repo_root is None:
        raise FileNotFoundError(f"Could not resolve repository root from {this_file}")
    ...
    if not isinstance(ref_choice_models, dict):
        ref_choice_models = ref_spec_raw.get("ref_state_models")
    if not isinstance(ref_choice_models, dict):
        raise ValueError(f"Missing or invalid ref_choice_models/ref_state_models in {ref_spec_path}")
...
def _load_specs_and_data(
    *,
    ...
    from beam_abm.decision_function.specs.model_plan import build_structural_backbone_plan
...
def run_backbone_models_fit_workflow(
    *,
    input_csv: Path = CSV_FILE_CLEAN,
    column_types_path: Path = DEFAULT_COLUMN_TYPES_PATH,
    derivations_path: Path | None = DEFAULT_DERIVATIONS_PATH,
    ...
    """Run the full structural backbone fit workflow."""
...
def run_backbone_models_block_importance_workflow(
    *,
    input_csv: Path = CSV_FILE_CLEAN,
    column_types_path: Path = DEFAULT_COLUMN_TYPES_PATH,
    derivations_path: Path | None = DEFAULT_DERIVATIONS_PATH,
    outdir: Path = DEFAULT_OUTDIR,
    models: str | None = None,
    baseline_blocks: str | None = "controls_demography_SES,health_history_engine",
    weight_column: str | None = None,
    standardize: bool = True,
    missingness_threshold: float = DEFAULT_MISSINGNESS_THRESHOLD,
    max_workers: int = 6,
    ...
    """Compute LOBO/LOBI block importance workflow."""
...
def run_backbone_models_workflow(
    *,
    input_csv: Path = CSV_FILE_CLEAN,
    column_types_path: Path = DEFAULT_COLUMN_TYPES_PATH,
    derivations_path: Path | None = DEFAULT_DERIVATIONS_PATH,
