...
def load_attribute_label_map(path: Path, *, field: str = "question") -> dict[str, str]:
    """Load a mapping from attribute key -> human-readable label.

    Supports:
    - Clean preprocessing spec: a JSON object with a top-level ``new_columns`` list
      where each item has an ``id`` and (typically) ``question``/``answer`` fields.
    - Generic JSON mappings: ``{ "<col>": "<label>", ... }`` or
      ``{ "<col>": { "question": "...", "answer": "..." }, ... }``.
    - Lists of objects with an ``id`` or ``column`` key.
    """

    data: Any = json.loads(path.read_text(encoding="utf-8"))

    def _maybe_add(out: dict[str, str], key: Any, value: Any) -> None:
        if not isinstance(key, str) or not key.strip():
            return
    ...
    if isinstance(data, dict) and isinstance(data.get("new_columns"), list):
        for item in data["new_columns"]:
            if not isinstance(item, dict):
                continue
            key = item.get("id")
            val = item.get(field)
            if val is None and field != "question":
                val = item.get("question")
            _maybe_add(out, key, val)
        return out
    ...
    if isinstance(data, list):
        for item in data:
            if not isinstance(item, dict):
                continue
            key = item.get("id") or item.get("column")
            val = item.get(field)
            if val is None and field != "question":
                val = item.get("question")
            _maybe_add(out, key, val)
    ...
    raise ValueError(f"Unsupported attribute label map JSON structure: {path}")
