...
def run_cli(argv: list[str] | None = None) -> None:
    ...
    if args.step == "calibrate":
        from beam_abm.llm_microvalidation.shared.calibrate_llm_predictions import run_cli as step_run_cli

        step_run_cli(rest)
        return

    if args.step == "apply-calibration":
        from beam_abm.llm_microvalidation.shared.apply_calibration_to_predictions import run_cli as step_run_cli
    ...
    from beam_abm.llm_microvalidation.behavioural_outcomes.support.baseline_reconstruction.compute_metrics import (
        run_cli as compute_metrics,
    )
    from beam_abm.llm_microvalidation.behavioural_outcomes.support.baseline_reconstruction.generate_prompts import (
        run_cli as generate_prompts,
    )
    from beam_abm.llm_microvalidation.behavioural_outcomes.support.baseline_reconstruction.run_sampling import (
        run_cli as run_sampling,
    )
    ...
    if args.step in {"metrics", "all"}:
        ...
        if args.step == "all":
            from beam_abm.llm_microvalidation.shared.calibrate_llm_predictions import run_cli as run_calibrate
